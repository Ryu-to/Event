...
if (isset($_FILES['upfile']) && $_FILES['upfile']['error'] == 0) {
  ...
  if (is_uploaded_file($temp_path)) {
    ...
    exit('Error:アップロードできませんでした'); // tmpフォルダにデータがない
  }
} else {
    exit('Error:画像がありません'); 
} 
} else {
  $sql = 'INSERT INTO event_table(id,event, image, description, category,event_area,address,person,hour,price,reserve_limit,mini_person) VALUES (NULL,:event, NULL, :description,:category,:event_area,:address,:person,:hour,:price,:reserve_limit,:mini_person)';
  $stmt = $pdo->prepare($sql);
  $stmt->bindValue(':event', $event, PDO::PARAM_STR);
  $stmt->bindValue(':image', $filename_to_save, PDO::PARAM_STR);
  $stmt->bindValue(':description', $description, PDO::PARAM_STR);
